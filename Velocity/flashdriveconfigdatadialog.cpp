...
FlashDriveConfigDataDialog::FlashDriveConfigDataDialog(FlashDriveConfigurationData *configData,
        QStatusBar *statusBar, QWidget *parent) :
    QDialog(parent), ui(new Ui::FlashDriveConfigDataDialog), statusBar(statusBar),
    configData(configData)
{
    ...
    ui->lblReadSpeed->setText(QString::fromStdString(ByteSizeToString(configData->readSpeed)) +
            "/second");
    ui->lblWriteSpeed->setText(QString::fromStdString(ByteSizeToString(configData->WriteSpeed)) +
            "/second");
    ...
    if (configData->securityLength == 0x228)
    {
        ...
        ui->txtSignature->setText(QtHelpers::ByteArrayToString(configData->certificate.signature, 0x80,
                true));
    }
    else
    {
