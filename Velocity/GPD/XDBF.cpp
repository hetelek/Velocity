...
DWORD XDBF::AllocateMemory(DWORD size)
{
	...
	else
	{
		...
		if (size != freeMemory.at(index).length)
			DeallocateMemory(GetRealAddress(freeMemory.at(index).addressSpecifier + size), freeMemory.at(index).length - size);

		// upadate the last free mem table entry
		freeMemory.at(freeMemory.size() - 1).addressSpecifier = totalFreeMemory;
		freeMemory.at(freeMemory.size() - 1).length = 0xFFFFFFFF - totalFreeMemory;
