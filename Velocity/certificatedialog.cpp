    ...
    QDialog(parent), ui(new Ui::CertificateDialog), cert(cert), statusBar(statusBar)
{
    ...
    ui->tableWidget->setItem(0, 0,
            new QTableWidgetItem("0x" + QString::number(cert->publicKeyCertificateSize, 16).toUpper()));

    // set the console id
    ui->tableWidget->setItem(1, 0,
            new QTableWidgetItem(QtHelpers::ByteArrayToString(cert->ownerConsoleID, 5, false)));

    // set the part number
    ui->tableWidget->setItem(2, 0,
            new QTableWidgetItem(QString::fromStdString(cert->ownerConsolePartNumber)));
    ...
    ui->tableWidget->setItem(7, 0, new QTableWidgetItem("0x" + QString::number(cert->publicExponent,
            16).toUpper()));
...
void CertificateDialog::on_pushButton_clicked()
{
    ...
    if (!QtHelpers::VerifyHexString(ui->tableWidget->item(0, 0)->text()))
    {
        QMessageBox::warning(this, "Invalid Value",
                "The Public Key Certificate Size must be all hexadecimal digits.\n");
        return;
    }
    if (ui->tableWidget->item(1, 0)->text().trimmed().length() != 10 ||
            !QtHelpers::VerifyHexStringBuffer(ui->tableWidget->item(1, 0)->text().trimmed()))
    {
    ...
    {
        QMessageBox::warning(this, "Invalid Value",
                "The length of the Console Part Number must be less than 20.\n");
        return;
    }
    ...
    {
        QMessageBox::warning(this, "Invalid Value",
                "The Public Exponent must be all hexadecimal digits.\n");
        return;
    }
    ...
    cert->consoleTypeFlags = (ConsoleTypeFlags)((cmbxTestKit->currentIndex() << 30) |
            (cmbxRecoveryGenerated->currentIndex() << 31));
