...
// Reverse lookup cache: Unicode → CP932 code (lazy initialized)
static QHash<char32_t, quint16> g_unicodeToCP932Cache;
...
static void initializeReverseCache() {
    ...
    g_unicodeToCP932Cache.reserve(cp932MapSize);
    for (size_t i = 0; i < cp932MapSize; ++i) {
        g_unicodeToCP932Cache.insert(
            cp932ToUnicodeMap[i].unicode,
            cp932ToUnicodeMap[i].code
        );
    }
...
QString CP932Decoder::decode(const QByteArray& data) const {
    QString result;
    ...
    while (i < size) {
        ...
        if (byte >= 0xA1 && byte <= 0xDF) {
            ...
            result.append(QChar(0xFF61 + (byte - 0xA1)));
            ++i;
            continue;
        }
        ...
        if (isLeadByte(byte)) {
            ...
            const quint16 sjis = (static_cast<quint16>(byte) << 8) | trail;
            ...
            // Invalid single byte (0x80-0xA0, 0xE0-0xFF without valid trail)
            result.append(QChar::ReplacementCharacter);
            ++i;
        }
    }
...
QByteArray CP932Decoder::encode(const QString& text) const {
    QByteArray result;
    ...
    for (const QChar& ch : text) {
        ...
        if (unicode >= 0xFF61 && unicode <= 0xFF9F) {
            result.append(static_cast<char>(0xA1 + (unicode - 0xFF61)));
            continue;
        }
        
        // Lookup in mapping table
        quint8 lead = 0, trail = 0;
        if (lookupCP932(unicode, lead, trail)) {
            if (lead == 0) {
...
bool CP932Decoder::isLeadByte(quint8 byte) {
    // CP932 lead byte ranges:
    // 0x81-0x9F, 0xE0-0xEF, 0xFA-0xFC
    return (byte >= 0x81 && byte <= 0x9F) ||
           (byte >= 0xE0 && byte <= 0xEF) ||
           (byte >= 0xFA && byte <= 0xFC);
}

quint16 CP932Decoder::lookupUnicode(quint8 lead, quint8 trail) {
    const quint16 code = (static_cast<quint16>(lead) << 8) | trail;
    
    // Use the mapping table's helper function
    QChar mapped = mapCP932ToUnicode(code);
...
bool CP932Decoder::lookupCP932(quint16 unicode, quint8& outLead, quint8& outTrail) {
    ...
    auto it = g_unicodeToCP932Cache.constFind(unicode);
    if (it == g_unicodeToCP932Cache.constEnd()) {
        return false;
    }
...
bool CP932Decoder::supports(QChar ch) const {
    const quint16 unicode = ch.unicode();
    ...
    quint8 lead = 0, trail = 0;
    return lookupCP932(unicode, lead, trail);
}
