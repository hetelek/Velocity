#include "CP950Decoder.h"
#include "CP950Mapping.h"
#include <QDebug>
#include <QHash>
#include <QMutex>
...
// Reverse lookup cache: Unicode → CP950 code (lazy initialized)
static QHash<char32_t, quint16> g_unicodeToCP950Cache;
...
static void initializeReverseCache() {
    ...
    g_unicodeToCP950Cache.reserve(cp950MapSize);
    for (size_t i = 0; i < cp950MapSize; ++i) {
        g_unicodeToCP950Cache.insert(
            cp950ToUnicodeMap[i].unicode,
            cp950ToUnicodeMap[i].code
        );
    }
...
QString CP950Decoder::decode(const QByteArray& data) const {
    QString result;
...
QByteArray CP950Decoder::encode(const QString& text) const {
    QByteArray result;
    ...
    for (const QChar& ch : text) {
        ...
        if (lookupCP950(unicode, lead, trail)) {
            if (lead == 0) {
...
bool CP950Decoder::isLeadByte(quint8 byte) {
    // CP950/Big5 lead byte range: 0xA1-0xF9
    return byte >= 0xA1 && byte <= 0xF9;
}

quint16 CP950Decoder::lookupUnicode(quint8 lead, quint8 trail) {
    const quint16 code = (static_cast<quint16>(lead) << 8) | trail;
    
    // Use the mapping table's helper function
    QChar mapped = mapCP950ToUnicode(code);
...
bool CP950Decoder::lookupCP950(quint16 unicode, quint8& outLead, quint8& outTrail) {
    ...
    auto it = g_unicodeToCP950Cache.constFind(unicode);
    if (it == g_unicodeToCP950Cache.constEnd()) {
        return false;
    }
...
bool CP950Decoder::supports(QChar ch) const {
    const quint16 unicode = ch.unicode();
    ...
    quint8 lead = 0, trail = 0;
    return lookupCP950(unicode, lead, trail);
}
