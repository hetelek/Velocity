#include "CP936Decoder.h"
#include "CP936Mapping.h"
#include <QDebug>
#include <QHash>
#include <QMutex>
...
// Reverse lookup cache: Unicode → CP936 code (lazy initialized)
static QHash<char32_t, quint16> g_unicodeToCP936Cache;
...
static void initializeReverseCache() {
    ...
    g_unicodeToCP936Cache.reserve(cp936MapSize);
    for (size_t i = 0; i < cp936MapSize; ++i) {
        g_unicodeToCP936Cache.insert(
            cp936ToUnicodeMap[i].unicode,
            cp936ToUnicodeMap[i].code
        );
    }
...
QString CP936Decoder::decode(const QByteArray& data) const {
    QString result;
    ...
    while (i < size) {
        ...
        if (isLeadByte(byte)) {
            ...
            // Invalid single byte (0x80 without valid trail)
            result.append(QChar::ReplacementCharacter);
            ++i;
        }
    }
...
QByteArray CP936Decoder::encode(const QString& text) const {
    QByteArray result;
    ...
    for (const QChar& ch : text) {
        ...
        if (lookupCP936(unicode, lead, trail)) {
            if (lead == 0) {
...
bool CP936Decoder::isLeadByte(quint8 byte) {
    // CP936/GBK lead byte range: 0x81-0xFE
    return byte >= 0x81 && byte <= 0xFE;
}

quint16 CP936Decoder::lookupUnicode(quint8 lead, quint8 trail) {
    const quint16 code = (static_cast<quint16>(lead) << 8) | trail;
    
    // Use the mapping table's helper function
    QChar mapped = mapCP936ToUnicode(code);
...
bool CP936Decoder::lookupCP936(quint16 unicode, quint8& outLead, quint8& outTrail) {
    ...
    auto it = g_unicodeToCP936Cache.constFind(unicode);
    if (it == g_unicodeToCP936Cache.constEnd()) {
        return false;
    }
...
bool CP936Decoder::supports(QChar ch) const {
    const quint16 unicode = ch.unicode();
    ...
    quint8 lead = 0, trail = 0;
    return lookupCP936(unicode, lead, trail);
}
