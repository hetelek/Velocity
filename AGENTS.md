...
CMake can download Botan when the submodule is absent; ensure your environment allows that fetch or preload `botan/`. Guard release artifacts by validating Qt paths via `qmake6 --version`, and never commit generated `botan_all.cpp` blobs from the build tree. For MSVC builds, ensure you run CMake from a Developer Command Prompt or Developer PowerShell for VS 2022 to have the compiler in your PATH.
